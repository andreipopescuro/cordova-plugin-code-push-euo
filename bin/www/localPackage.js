...
var LocalPackage = (function (_super) {
  ...
  LocalPackage.prototype.install = function (
    installSuccess,
    errorCallback,
    installOptions
  ) {
    ...
    try {
      ...
      var newPackageUnzipped = function (unzipError) {
        if (unzipError) {
          ...
          LocalPackage.handleDeployment(
            newPackageLocation,
            CodePushUtil.getNodeStyleCallbackFor(
              donePackageFileCopy,
              installError
            )
          );
      ...
      FileUtil.getDataDirectory(
        ...
        false,
        function (error, directoryEntry) {
          var unzipPackage = function () {
            FileUtil.getDataDirectory(
              LocalPackage.DownloadUnzipDir,
              true,
              function (innerError, unzipDir) {
                if (innerError) {
          ...
          if (!error && !!directoryEntry) {
            directoryEntry.removeRecursively(
              ...
              function (cleanupError) {
                installError &&
                  installError(FileUtil.fileErrorToError(cleanupError));
              }
            );
          } else {
            unzipPackage();
  ...
  LocalPackage.prototype.verifyPackage = function (
    deploymentResult,
    installError,
    successCallback
  ) {
    ...
    this.getPublicKey(function (error, publicKeyResult) {
      if (error) {
        installError &&
          installError(new Error("Error reading public key. " + error));
        return;
      }
      publicKey = publicKeyResult;
      isSignatureVerificationEnabled = !!publicKey;
      _this.getSignatureFromUpdate(
        deploymentResult.deployDir,
        function (error, signature) {
          if (error) {
            installError &&
              installError(
                new Error("Error reading signature from update. " + error)
              );
            return;
          }
  ...
  LocalPackage.prototype.getSignatureFromUpdate = function (
    deployDir,
    callback
  ) {
  ...
  LocalPackage.prototype.verifyHash = function (
    deployDir,
    newUpdateHash,
    errorCallback,
    successCallback
    ...
    var packageHashSuccess = function (computedHash) {
      if (computedHash !== newUpdateHash) {
        errorCallback(
          new Error("The update contents failed the data integrity check.")
        );
        return;
      }
      CodePushUtil.logMessage(
    ...
    CodePushUtil.logMessage(
      "Verifying hash for folder path: " + deployDir.fullPath
    );
    cordova.exec(
      packageHashSuccess,
      packageHashFail,
  ...
  LocalPackage.prototype.verifySignature = function (
    deployDir,
    newUpdateHash,
    publicKey,
    signature,
    ...
    var decodeSignatureSuccess = function (contentHash) {
      if (contentHash !== newUpdateHash) {
        errorCallback(
          new Error("The update contents failed the code signing check.")
        );
        return;
      }
      CodePushUtil.logMessage(
    ...
    CodePushUtil.logMessage(
      "Verifying signature for folder path: " + deployDir.fullPath
    );
    cordova.exec(
      decodeSignatureSuccess,
      decodeSignatureFail,
      "CodePush",
      "decodeSignature",
      [publicKey, signature]
  ...
  LocalPackage.prototype.finishInstall = function (
    deployDir,
    installOptions,
    installSuccess,
    installError
        ...
        _this.writeNewPackageMetadata(deployDir, function (writeMetadataError) {
          if (writeMetadataError) {
            ...
            var invokeSuccessAndInstall = function () {
              ...
              var installModeToUse = _this.isMandatory
            ...
            var preInstallFailure = function (preInstallError) {
              ...
              var error = new Error(
                "An error has occurred while installing the package. " +
                  CodePushUtil.getErrorMessage(preInstallError)
              );
              installError && installError(error);
            };
            cordova.exec(
  ...
  LocalPackage.handleDeployment = function (
    newPackageLocation,
    deployCallback
  ) {
    FileUtil.getDataDirectory(
      newPackageLocation,
      true,
      function (deployDirError, deployDir) {
        FileUtil.getDataFile(
          LocalPackage.DownloadUnzipDir,
          LocalPackage.DiffManifestFile,
          false,
          function (manifestError, diffManifest) {
            if (!manifestError && !!diffManifest) {
              ...
              LocalPackage.handleCleanDeployment(
                newPackageLocation,
                function (error) {
  ...
  LocalPackage.prototype.writeNewPackageMetadata = function (
    deployDir,
    writeMetadataCallback
  ) {
    var _this = this;
    NativeAppInfo.getApplicationBuildTime(function (buildTimeError, timestamp) {
      NativeAppInfo.getApplicationVersion(function (
        appVersionError,
        appVersion
      ) {
        buildTimeError &&
  ...
  LocalPackage.handleCleanDeployment = function (
    newPackageLocation,
    cleanDeployCallback
  ) {
    FileUtil.getDataDirectory(
      ...
      function (deployDirError, deployDir) {
        FileUtil.getDataDirectory(
          LocalPackage.DownloadUnzipDir,
          false,
          function (unzipDirErr, unzipDir) {
            if (unzipDirErr || deployDirError) {
              cleanDeployCallback(
                new Error("Could not copy new package."),
                null
              );
            } else {
              FileUtil.copyDirectoryEntriesTo(
                unzipDir,
                deployDir,
                [],
                function (copyError) {
  ...
  LocalPackage.copyCurrentPackage = function (
    newPackageLocation,
    ignoreList,
    copyCallback
  ) {
    ...
    var doCopy = function (currentPackagePath) {
      ...
      if (currentPackagePath) {
        getCurrentPackageDirectory = function (
          getCurrentPackageDirectoryCallback
        ) {
          FileUtil.getDataDirectory(
            currentPackagePath,
            false,
      ...
      FileUtil.getDataDirectory(
        ...
        function (deployDirError, deployDir) {
          if (deployDirError) {
            handleError(
              new Error("Could not acquire the source/destination folders. ")
            );
          } else {
            ...
            getCurrentPackageDirectory(
              CodePushUtil.getNodeStyleCallbackFor(success, fail)
            );
          }
        }
      );
  ...
  LocalPackage.handleDiffDeployment = function (
    newPackageLocation,
    diffManifest,
    diffCallback
  ) {
    ...
    LocalPackage.copyCurrentPackage(
      newPackageLocation,
      [".codepushrelease"],
      function (currentPackageError) {
        LocalPackage.handleCleanDeployment(
          newPackageLocation,
          function (cleanDeployError) {
            FileUtil.readFileEntry(diffManifest, function (error, content) {
              if (error || currentPackageError || cleanDeployError) {
                ...
                FileUtil.deleteEntriesFromDataDirectory(
                  newPackageLocation,
                    ...
                    FileUtil.getDataDirectory(
                      ...
                      function (deployDirError, deployDir) {
                        if (deleteError || deployDirError) {
                          handleError(
                            new Error("Cannot clean up deleted manifest files.")
                          );
                        } else {
                          diffCallback(null, {
                            deployDir: deployDir,
  ...
  LocalPackage.writeCurrentPackageInformation = function (
    packageInfoMetadata,
    callback
  ) {
  ...
  LocalPackage.getOldPackage = function (packageSuccess, packageError) {
    return LocalPackage.getPackage(
      LocalPackage.OldPackageInfoFile,
      packageSuccess,
      packageError
    );
  };
  LocalPackage.getPackage = function (
    packageFile,
    packageSuccess,
    packageError
  ) {
        ...
        function (error, content) {
          if (error) {
            ...
            try {
              ...
              LocalPackage.getLocalPackageFromMetadata(
                packageInfo,
                packageSuccess,
                packageError
              );
  ...
  LocalPackage.getLocalPackageFromMetadata = function (
    metadata,
    packageSuccess,
    packageError
  ) {
  ...
  LocalPackage.getCurrentOrDefaultPackage = function (
    packageSuccess,
    packageError
  ) {
    LocalPackage.getPackageInfoOrDefault(
      LocalPackage.PackageInfoFile,
      packageSuccess,
      packageError
    );
  };
  LocalPackage.getOldOrDefaultPackage = function (
    packageSuccess,
    packageError
  ) {
    LocalPackage.getPackageInfoOrDefault(
  ...
  LocalPackage.getPackageInfoOrDefault = function (
    packageFile,
    packageSuccess,
    packageError
  ) {
  ...
  LocalPackage.getPackageInfoOrNull = function (
    packageFile,
    packageSuccess,
    packageError
  ) {
