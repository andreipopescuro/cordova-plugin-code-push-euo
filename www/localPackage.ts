...
class LocalPackage extends Package implements ILocalPackage {
  ...
  public install(
    ...
    try {
      ...
      var newPackageUnzipped = function (unzipError: Error) {
        if (unzipError) {
          ...
          LocalPackage.handleDeployment(
            newPackageLocation,
            CodePushUtil.getNodeStyleCallbackFor<DeploymentResult>(
              donePackageFileCopy,
              installError
            )
          );
      ...
      FileUtil.getDataDirectory(
        ...
        false,
        (error: Error, directoryEntry: DirectoryEntry) => {
          var unzipPackage = () => {
            FileUtil.getDataDirectory(
              LocalPackage.DownloadUnzipDir,
              true,
              (innerError: Error, unzipDir: DirectoryEntry) => {
                if (innerError) {
  ...
  private finishInstall(
    ...
    function backupPackageInformationFileIfNeeded(
        ...
        this.writeNewPackageMetadata(deployDir, (writeMetadataError: Error) => {
          if (writeMetadataError) {
            ...
            var invokeSuccessAndInstall = () => {
              ...
              var installModeToUse: InstallMode = this.isMandatory
  ...
  private static handleDeployment(
    ...
    deployCallback: Callback<DeploymentResult>
  ): void {
    FileUtil.getDataDirectory(
      newPackageLocation,
      true,
      (deployDirError: Error, deployDir: DirectoryEntry) => {
        // check for diff manifest
        FileUtil.getDataFile(
          LocalPackage.DownloadUnzipDir,
          LocalPackage.DiffManifestFile,
          false,
          (manifestError: Error, diffManifest: FileEntry) => {
  ...
  private writeNewPackageMetadata(
    deployDir: DirectoryEntry,
    writeMetadataCallback: Callback<void>
  ): void {
    NativeAppInfo.getApplicationBuildTime(
      (buildTimeError: Error, timestamp: string) => {
        NativeAppInfo.getApplicationVersion(
          (appVersionError: Error, appVersion: string) => {
            buildTimeError &&
  ...
  private static handleCleanDeployment(
    newPackageLocation: string,
    cleanDeployCallback: Callback<DeploymentResult>
  ): void {
    // no diff manifest
    FileUtil.getDataDirectory(
      newPackageLocation,
      ...
      (deployDirError: Error, deployDir: DirectoryEntry) => {
        FileUtil.getDataDirectory(
          LocalPackage.DownloadUnzipDir,
          false,
          (unzipDirErr: Error, unzipDir: DirectoryEntry) => {
            if (unzipDirErr || deployDirError) {
              ...
              FileUtil.copyDirectoryEntriesTo(
                ...
                deployDir,
                [
                  /*no need to ignore copy anything*/
                ],
                (copyError: Error) => {
                  if (copyError) {
  ...
  private static copyCurrentPackage(
    ...
    var doCopy = (currentPackagePath?: string) => {
      var getCurrentPackageDirectory: (
        getCurrentPackageDirectoryCallback: Callback<DirectoryEntry>
      ) => void;
  ...
  private static handleDiffDeployment(
    newPackageLocation: string,
    diffManifest: FileEntry,
    diffCallback: Callback<DeploymentResult>
  ): void {
    var handleError = (e: Error) => {
      diffCallback(e, null);
    };
    ...
    LocalPackage.copyCurrentPackage(
      newPackageLocation,
      [".codepushrelease"],
      (currentPackageError: Error) => {
        /* copy new files */
        LocalPackage.handleCleanDeployment(
          newPackageLocation,
          (cleanDeployError: Error) => {
            /* delete files mentioned in the manifest */
            FileUtil.readFileEntry(
              diffManifest,
                      ...
                      FileUtil.getDataDirectory(
                        ...
                        (deployDirError: Error, deployDir: DirectoryEntry) => {
                          if (deleteError || deployDirError) {
                            handleError(
                              new Error(
                                "Cannot clean up deleted manifest files."
                              )
                            );
                          } else {
                            diffCallback(null, {
    ...
    var gotFile = (error: Error, currentPackageFile: FileEntry) => {
      if (error) {
        ...
        FileUtil.getDataFile(
          ...
          LocalPackage.OldPackageInfoFile,
          false,
          (error: Error, oldPackageFile: FileEntry) => {
  ...
  public static getCurrentOrDefaultPackage(
    packageSuccess: SuccessCallback<LocalPackage>,
    packageError?: ErrorCallback
  ): void {
  ...
  public static getOldOrDefaultPackage(
    packageSuccess: SuccessCallback<LocalPackage>,
    packageError?: ErrorCallback
  ): void {
    LocalPackage.getPackageInfoOrDefault(
  ...
  public static getPackageInfoOrDefault(
    packageFile: string,
    packageSuccess: SuccessCallback<LocalPackage>,
    packageError?: ErrorCallback
  ): void {
    var packageFailure = (error: Error) => {
      NativeAppInfo.getApplicationVersion(
        (appVersionError: Error, appVersion: string) => {
          ...
          NativeAppInfo.getBinaryHash(
            (binaryHashError: Error, binaryHash: string) => {
