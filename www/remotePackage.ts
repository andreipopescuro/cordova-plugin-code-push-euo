      ...
      (error: Error, _: DirectoryEntry) => {
        ...
        if (error) {
          CodePushUtil.logError(
            "Can't create directory for download update.",
            error
          );
        }
      }
  ...
  public download(
    ...
    try {
      ...
      if (!this.downloadUrl) {
        ...
        const onFileError: FileSaverErrorHandler = (
          fileError: FileError,
          stage: string
        ) => {
          const error = new Error(
            "Could not access local package. Stage:" +
              stage +
              "Error code: " +
              fileError.code
          );
        ...
        const onFileReady: FileSaverCompletionHandler = (
          fileEntry: FileEntry
        ) => {
          this.isDownloading = false;
            ...
            (file: File) => {
              NativeAppInfo.isFailedUpdate(
                ...
                (installFailed: boolean) => {
                  ...
                  localPackage.localPath = fileEntry.toNativeURL();
        ...
        const filedir =
          cordova.file.dataDirectory + LocalPackage.DownloadDir + "/";
      ...
      CodePushUtil.invokeErrorCallback(
        new Error(
          "An error occurred while downloading the package. " + (e && e.message)
            ? e.message
            : ""
        ),
        errorCallback
